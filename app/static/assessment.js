...
function render(){
 ...
 if(q.type==='single') html=q.options.map(o=>`<label class="option"><input type="radio" name="v" value="${o}" ${a[q.id]===o?'checked':''}> ${pretty(o)}</label>`).join('');
  ...
  html=pairs.map((p,k)=>`<div class="card"><label class="option"><input type="radio" name="p${k}" value="${p[0]}"> ${pretty(p[0])}</label><label class="option"><input type="radio" name="p${k}" value="${p[1]}"> ${pretty(p[1])}</label></div>`).join('');
 }
