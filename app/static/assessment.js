...
async function init(){
 ...
 if(!access.ok){$('#app').innerHTML='<div class="card"><h2>Access not found</h2><a class="btn" href="/">Return home</a></div>';return}
...
function render(){
 const q=qs[i]; $('#step').textContent='Question '+(i+1)+' of '+qs.length; $('#progress').style.width=Math.round(i/qs.length*100)+'%';
 let html='';
 ...
 $('#question').innerHTML=`<div class="eyebrow">Path Assessment</div><h1>${q.prompt}</h1><div class="options">${html}</div><div id="err" class="error"></div><div class="navrow"><button class="btn secondary" id="back" ${i===0?'disabled':''}>Back</button><button class="btn" id="next">${i===qs.length-1?'Build my path report':'Continue'}</button></div>`;
 $('#back').onclick=()=>{capture(false);i--;render()}; $('#next').onclick=next;
}
function capture(validate=true){
 ...
 if(q.type==='text')v=$('#txt').value.trim();
 ...
 if(validate && q.type==='single' && v===undefined){$('#err').textContent='Choose an answer to continue.';return false}
 if(validate && q.type==='scale_1_5' && v===undefined){$('#err').textContent='Choose an answer to continue.';return false}
 if(validate && q.id==='primary_objectives' && (!v.length||v.length>2)){ $('#err').textContent='Choose one or two options.';return false}
 if(validate && q.id==='natural_activities' && v.length>5){$('#err').textContent='Choose up to five options.';return false}
 if(validate && q.type==='forced_pairs' && Object.keys(v).length<4){$('#err').textContent='Choose one option in each pair.';return false}
 if(q.id==='experience_areas'&&v.includes('none')&&v.length>1){$('#err').textContent='Choose None by itself.';return false}
 if(q.id==='constraints'&&v.includes('none')&&v.length>1){$('#err').textContent='Choose None by itself.';return false}
 a[q.id]=v;return true
}
async function next(){
 if(!capture(true))return;
 if(i<qs.length-1){i++;render();return}
 $('#question').innerHTML='<div class="loading"><h2>Building your path report…</h2></div>';
 ...
 const j=await r.json(); if(!r.ok){$('#question').innerHTML='<div class="card"><h2>Could not build report</h2><p class="error">'+(j.detail||'Try again')+'</p></div>';return}
