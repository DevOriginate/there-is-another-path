...
function metaConsent(id){if(!id)return;const saved=localStorage.getItem('meta-ads-consent');if(saved==='accepted'){installPixel(id);return}if(saved==='declined')return;const box=document.createElement('div');box.className='consent-banner';box.innerHTML='<div><strong>Advertising measurement</strong><p>We use optional Meta advertising measurement only with your choice. It never changes your Path recommendation.</p></div><div class="consent-actions"><button class="btn secondary" data-no>Decline</button><button class="btn" data-yes>Accept</button></div>';document.body.appendChild(box);box.querySelector('[data-yes]').onclick=()=>{localStorage.setItem('meta-ads-consent','accepted');box.remove();installPixel(id)};box.querySelector('[data-no]').onclick=()=>{localStorage.setItem('meta-ads-consent','declined');box.remove()}}
async function initCommon(){const c=await config(); metaConsent(c.meta_pixel_id); document.querySelectorAll('.support-email').forEach(x=>{x.textContent=c.support_email;x.href='mailto:'+c.support_email}); return c}
async function beginCheckout(){const c=await initCommon(); fire('InitiateCheckout',{value:c.product_price_usd,currency:'USD'}); const r=await fetch('/api/v1/checkout/session',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({acquisition:acquisition()})}); const j=await r.json(); if(!r.ok){alert(j.detail||'Checkout failed');return} location.href=j.url}
...
async function initLanding(){
  ...
  $('.price-value').forEach(x=>x.textContent='
window.PathFinder={config,fire,initCommon};
window.addEventListener('DOMContentLoaded',()=>{if(document.body.dataset.page==='landing')initLanding();else initCommon()})+c.product_price_usd);
...
window.PathFinder={config,fire,initCommon};
window.addEventListener('DOMContentLoaded',()=>{if(document.body.dataset.page==='landing')initLanding();else initCommon()})